        ...
        <div class="my-audio">
                    ...
                    <div class="res-bottom">
                        <ol class="show-li" v-show="isA">
                            <li @click="addChose($event)" v-for="f in Audiofiles" :value="f.resid" class="lis">
                                 <div>
                                        <p class="imgs audio" v-if="f.status==0"></p>
                                        <p class="imgs" v-else :class="$parent.getTransCodingStatus(f.rtid,f.status)"></p>
                                        <em>{{getTime(f.duration)}}</em>
                                        <p :title="f.restitle">{{f.restitle}}</p>
                                </div>
                                <p class="instruction">
                                        <span class="left" :class="[f.type == 2?'compositing':(f.type==1?'up-s':'collect-video')]">
                                       {{f.type ==2?'合成音频':(f.type ==1?'上传音频':'收藏音频')}}</span>
                                        <span class="s-right" :class="[f.public===1?'open-s':'own-s']">{{f.public==1?'公开':'私有'}}</span>
                                </p>
                            </li>
        ...
        export default {
                name:"my-audio",
                        ...
                        watch:{
                            ids:function(newval,oldval){
                                ...
                                if(this.ids.length>1){
                                     $("a.onplay").addClass('blue-none').removeClass('blue');
                                ...
                                if(this.ids.length == 1){
                                    for(var i = 0; i < this.Audiofiles.length; i++){
                                        if(this.Audiofiles[i].resid == this.ids[0]){
                                            if(this.Audiofiles[i].public == 1){
                                ...
                                if(this.ids.length < this.Audiofiles.length||this.ids.length == 0){
                                    $("#checkAll").prop("checked",false);
                                }else if(this.ids.length > 0&&this.ids.length){
                        ...
                        computed:{
                            ...
                            _upload:function(){
                                return handleAuth(71,this.handles);
                            },
                            _move:function(){
                                return handleAuth(72,this.handles);
                            },
                            _copy:function(){
                                return handleAuth(73,this.handles);
                            },
                            _delete:function(){
                                return handleAuth(74,this.handles);
                            },
                            _download:function(){
                                return handleAuth(75,this.handles);
                            },
                            _share:function(){
                                return handleAuth(76,this.handles);
                            },
                            _play:function(){
                                return handleAuth(77,this.handles);
                            },
                            _public:function(){
                                return handleAuth(78,this.handles);
                            }
                        },
                         mounted:function(){
                                this.getAudioFile();
                        ...
                        methods:{
                            ...
                            checkAll(){
                                if($("#checkAll").prop("checked")){
                                    this.ids = [];
                                    for(var i = 0; i < this.Audiofiles.length; i++){
                                        this.ids.push(this.Audiofiles[i].resid)
                                    }                                    
                                    var span = `<span class="choseSure"></span>`;
                                    $(".lis").append(span);
                                }else{
                            ...
                            downLoad(){
                                ...
                                for(var i = 0; i < this.Audiofiles.length; i++){
                                    if(this.Audiofiles[i].resid == this.ids[0]){
                                        this.md5 = this.Audiofiles[i].md5
                                    }
                                }
                                this.$http.get('/storage/getfilepath',{
                                        params:{
                            ...
                            toggleUp(){
                                if ($('a.up').hasClass('blue')) {
                                        $(".audio-opration a").addClass('blue-none').removeClass('blue');
                                        $("a.up").addClass('blue').removeClass('blue-none');
                                        this.$parent.createUpload(0,0,'全部资源\\我的收藏\\音频',2); 
                                }
                            },
                            // 上传文件弹窗隐藏
                            closeUp(){
                            ...
                            goToPlay(){
                                if($("a.onplay").hasClass('blue-none')){
                                 ...
                                 this.$router.push({path:'resource/audio/detail',query:{id:this.ids[0]} });
                            },
                            edit(){
                                 if ($('a.edit').hasClass('blue')) {
                                        $(".audio-opration a").addClass('blue-none').removeClass('blue');
                                        $("a.edit").addClass('blue').removeClass('blue-none');
                                        this.$parent.createUpload(this.ids[0],0,'全部资源\\我的收藏\\音频',2);
                                }
                            },
                             getUploading(){
                                this.uploadings=[];
